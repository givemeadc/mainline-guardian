...
class GoalGuardTests(unittest.TestCase):
    def test_init_and_checkpoint(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run(root, "init", "--title", "Build skill", "--objective", "Create a goal alignment skill", "--observe", "Skill is usable in a fresh task")
    ...
    def test_branch_close_emits_return_sentence(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run(root, "init", "--title", "Main", "--objective", "Finish the main task", "--observe", "Main task works")
            run(root, "branch", "open", "--kind", "exploration", "--reason", "Inspect another tool", "--exit-criteria", "Record one conclusion", "--return-to", "main task")
            result = run(root, "branch", "close", "--id", "B1", "--result-type", "evidence", "--result", "Tool is compatible", "--remaining", "implementation", "--next", "implement main task")
    ...
    def test_explicit_switch_is_recorded(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run(root, "init", "--title", "Main", "--objective", "Finish A", "--observe", "A works")
            run(root, "goal", "add", "--title", "Second", "--objective", "Finish B")
            run(root, "goal", "switch", "--goal-id", "G2", "--reason", "explicit user request")
            state = json.loads((root / ".mainline" / "state.json").read_text(encoding="utf-8"))
    ...
    def test_decision_is_written_before_execution(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run(root, "init", "--title", "Main", "--objective", "Finish A", "--observe", "A works")
            run(root, "decision", "record", "--proposal", "A", "--mainline-relation", "reduces latency", "--ledger-refs", "v3 no gain", "--cost-risk", "medium", "--decision", "test")
            lines = (root / ".mainline" / "ledger.jsonl").read_text(encoding="utf-8").splitlines()
    ...
    def test_audit_rejects_missing_attestation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            run(root, "criterion", "close", "--id", "C1", "--observed", "Task works in environment", "--object", "app-v1")
